...
function getCurrentRegistration(value) {
...
function setButtonFeedback(button, { text, className = "is-pressed", duration = 180 } = {}) {
  if (!button) return;
  ...
  window.setTimeout(() => {
    ...
    if (typeof text === "string") button.textContent = originalText;
  }, duration);
}

function selectedFilterCount(key) {
  const value = activeFilters[key];
  return Array.isArray(value) ? value.length : 0;
}

function filterMatchesSelected(value, selectedValues) {
  if (!selectedValues.length) return true;
  const normalized = normalizeFilterValue(value);
  return selectedValues.some(item => normalizeFilterValue(item) === normalized);
}
...
function renderFilterButtons() {
  ...
  for (const key of Object.keys(activeFilters)) {
    ...
    activeFilters[key] = (activeFilters[key] || []).filter(value => options.includes(value));
  }

  els.filterStrip.innerHTML = FILTER_CONFIG.map(config => {
    const value = getFilterLabelValue(config.key);
    const selected = selectedFilterCount(config.key) > 0;
    return `
      <button
...
function openFilterPopover(key) {
  ...
  els.filterPopoverTitle.textContent = selected.length ? `Change ${config.label.toLowerCase()}` : `Choose ${config.label.toLowerCase()}`;
  els.filterPopoverList.innerHTML = [
    `<button class="filter-option${selected.length === 0 ? " active" : ""}" type="button" data-value="all">All ${escapeHtml(config.label.toLowerCase())}</button>`,
    ...options.map(option => `
      <button class="filter-option${selected.includes(option) ? " active" : ""}" type="button" data-value="${escapeHtml(option)}">
        ${escapeHtml(option)}
      </button>
    `)
  ].join("");
  ...
  els.filterPopoverList.querySelectorAll("[data-value]").forEach(button => {
    button.addEventListener("click", () => {
      ...
      if (value === "all") {
        ...
        activeFilters[key] = current.filter(item => item !== value);
      } else {
...
function currentFilteredFlights() {
  ...
  return rows.filter(row => {
    ...
    const yearOk = filterMatchesSelected(rowYear, activeFilters.year || []);
    const airlineOk = filterMatchesSelected(row.airline, activeFilters.airline || []);
    const whoOk = !selectedFilterCount("who") || splitPeople(row.who).some(person => filterMatchesSelected(person, activeFilters.who || []));
    const fromOk = filterMatchesSelected(row.from_airport, activeFilters.from_airport || []);
    const toOk = filterMatchesSelected(row.to_airport, activeFilters.to_airport || []);
    const airplaneOk = filterMatchesSelected(row.airplane, activeFilters.airplane || []);
    const flightOk = filterMatchesSelected(row.flight_code, activeFilters.flight_code || []);
...
function render() {
  ...
  els.grid.innerHTML = rows.map(row => {
    ...
    const reg = escapeHtml(row.registration || "No reg");
    const regValue = row.registration || "";
    ...
    return `
      <article class="flight-card">
        <div class="flight-head">
          ...
          <button class="pill reg reg-link" type="button" onclick='openAircraftOnFr24(${jsString(regValue)}, this)'>${reg}</button>
        </div>
...
async function copyVisibleRegistrations(button = els.copyRegsBtn) {
  const regs = [...new Set(
    currentFilteredFlights()
      .map(row => getCurrentRegistration(row.registration))
      .filter(Boolean)
  )];
  const text = regs.join(", ");

  if (!text) {
    setButtonFeedback(button, { text: "No regs", className: "is-copied", duration: 900 });
    return;
  }
  ...
  setButtonFeedback(button, { text: "Copied!", className: "is-copied", duration: 900 });
}

function openAircraftOnFr24(reg, button = null) {
  const cleanReg = getCurrentRegistration(reg);
  if (!cleanReg) return;
  setButtonFeedback(button, { className: "is-pressed", duration: 180 });
  window.open(`https://www.flightradar24.com/data/aircraft/${encodeURIComponent(cleanReg)}`, "_blank", "noopener,noreferrer");
}

window.openEdit = openEdit;
window.removeFlight = removeFlight;
window.openAircraftOnFr24 = openAircraftOnFr24;

els.addFlightBtn.addEventListener("click", () => showModal("add"));
els.refreshBtn.addEventListener("click", loadFlights);
els.copyRegsBtn.addEventListener("click", (e) => copyVisibleRegistrations(e.currentTarget));
